...
int InitVM()
{
	...
	/*
	 * this bit of code is used to reset a given node and
	 * neighbours to a specified concentration.
	 * Models a constant source.
	 */
    reset = (start!=NO_NB);
    if (reset) {
...
int cnt=0;
    ...
    if (EllemaxStages()==1) {
         ...
         */
        if ((Minlen*Minlen/KappaValue*0.5) < 1.0)
            fprintf(stderr,
    ...
    for (i=0;i<EllemaxStages();i++) {
      for (k=0;k<steps;k++) {
		...
		/*
		 * this bit of code is used to reset a given node and
		 * neighbours to a specified concentration at each step.
		 * Models a constant source.
		 */
        if (reset) {
