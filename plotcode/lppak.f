...
C
      IF(LDASH.LE.0.OR.LDASH.EQ.8)THEN
      WRITE(IWORD,101)
  101 FORMAT('[] 0 setdash ')  
      CALL PSEND(IWORD,13,1)
C
      ELSEIF(LDASH.EQ.1)THEN
      IDA(1)=LPAT*12
      IDA(2)=LPAT*4
      WRITE(IWORD,102)(IDA(J),J=1,2)
      CALL PSEND(IWORD,20,1)
C
...
C
  ...
  103 FORMAT('[',I3,' ',I3,' ',I3,' ',I3,'] 0 setdash ')
...
C     CALL PSEND(IWORD,11,1)
  ...
  106 FORMAT('/xcentre {stringwidth pop -2 div } def ')
      ...
      WRITE(IWORD,201)
C 201 FORMAT('/pF {lineto gsave fill grestore} def ')
  201 FORMAT('/pF {lineto fill } def ')
      CALL PSEND(IWORD,37,2)
      WRITE(IWORD,202)
  ...
  204 FORMAT('/pgh ',F7.2,' def ')
      ...
      WRITE(IWORD,205)
  205 FORMAT('/circle')
      CALL PSEND(IWORD,7,2)
      WRITE(IWORD,206)
  206 FORMAT('{/col exch def ')
      CALL PSEND(IWORD,15,2)
      WRITE(IWORD,207)
  207 FORMAT('/rad exch def ')
      CALL PSEND(IWORD,14,2)
      WRITE(IWORD,208)
  208 FORMAT('/ypos exch def ')
      CALL PSEND(IWORD,15,2)
      WRITE(IWORD,209)
  209 FORMAT('/xpos exch def ')
      CALL PSEND(IWORD,15,2)
      WRITE(IWORD,210)
  210 FORMAT('/c currentcolor def col setcolor ')
      CALL PSEND(IWORD,33,2)
      WRITE(IWORD,211)
  211 FORMAT('xpos ypos rad 0 360 arc fill ')
      CALL PSEND(IWORD,29,2)
      WRITE(IWORD,212)
  212 FORMAT('c setcolor')
      CALL PSEND(IWORD,11,2)
      WRITE(IWORD,213)
  213 FORMAT('}def')
      CALL PSEND(IWORD,4,2)
      WRITE(IWORD,109)
  109 FORMAT('newpath ')
...
C   
      CHARACTER*1 IWORD,ISEND,IBLANK
      COMMON/P00000/LPLOT,IROT,IL34,A,B,C,D,ASP,THET
      DIMENSION IWORD(480),ISEND(NC)
      save
      DATA INDEX/1/ 
      DATA IBLANK/' '/  
C   
...
C   
      ...
      WRITE(LPLOT,55)(IWORD(K),K=I1,I2) 
   55 FORMAT(80A1)  
   18 CONTINUE  
C   
...
C-----------------------------------------------------------le-
      ...
      CHARACTER*40 BUF
      CHARACTER*12 NUMBER
      DIMENSION PWINDO(IPWINDOWENTRIES)
      INTEGER OFFX, OFFY
      COMMON/P00000/LPLOT,IROT,IL34,A,B,C,D,ASP,THET
...
C
      ...
      INDX=1
      IF (OFFX.EQ.JRIGHT) THEN
        ICOPY(INDX:INDX+9)='dup right '
        INDX=INDX+10
      ELSE IF (OFFX.EQ.JCENTRE) THEN
      ...
      ELSE 
        ...
        INDX=INDX+2
      END IF
      IF (OFFY.EQ.JCENTRE) THEN 
        WRITE(BUF,20)-0.4
      ELSE IF (OFFY.EQ.JTOP) THEN
        WRITE(BUF,20)-1.0
      ELSE IF (OFFY.EQ.JBASE) THEN
        WRITE(BUF,20)0.25
      ELSE
        WRITE(BUF,30)0.0
      END IF
...
C             WRITE(BUF,60)
C  60         FORMAT('gsave ')
C             CALL PSEND(BUF,6,1)
              CALL SETCOLOUR(ICOLOUR)
              WRITE(BUF,70)
C  70         FORMAT('fill grestore stroke')
   70         FORMAT('fill ')
              CALL PSEND(BUF,5,1)
            ELSE
              WRITE(BUF,80)
...
C        max valid index in the range 0->val
 ...
 1000 FORMAT('[/Indexed /DeviceRGB ',I4,' <')
      ...
      DO 5 I=1,ISIZE
        ...
        DO 25 J=1,6
  25      IF (HEXNUM(J:J).EQ.' ') HEXNUM(J:J)='0'
 ...
 10   CONTINUE
      ...
      DO 15 I=1,INTERVALS
        ...
        DO 20 K = NC0,NC1
          ...
          DO 30 J=1,6
  30        IF (HEXNUM(J:J).EQ.' ') HEXNUM(J:J)='0'
          BUF(INDX:INDX+6) = HEXNUM(1:7)
          INDX = INDX + 7
          IF (INDX.GE.70) THEN
            CALL PSEND(BUF,INDX-1,1)
 ...
 1002 FORMAT(' > ] setcolorspace ')
      ...
      END
c-----------------------------------------------------------------bk
      SUBROUTINE DRAWTICMARKS(SIZE,RMAX,VAL,START,RINTRVL,
     1                            IJUSTIFY,IHORIZ)   
C
...
C
      ...
      ROFFSET = 0
C     don't change the value passed as SIZE
      TMPSIZE = SIZE
      IF (IJUSTIFY.EQ.JCENTRE) ROFFSET = -SIZE/2
      IF (IJUSTIFY.EQ.JTOP.OR.IJUSTIFY.EQ.JRIGHT)
     1  TMPSIZE = -SIZE
      NUM = (RMAX-START)/RINTRVL
      NUM = NUM+1
      IF (IHORIZ.NE.0) THEN
        XVAL = START
        DO 10 I=1,NUM
...
c-----------------------------------------------------------------bk
      SUBROUTINE DRAWSEGMENT(XU1,YU1,XU2,YU2,ICOL)   
C
C     draws a line segment in the given colour
C     co-ordinates in user units
C
      CHARACTER*30 BUF
...
C   
      ...
      K=I1 + (I-1)*I2   
   16 TM2(I,J)=TM(K,J)  
   18 TC2(J)=XYZCD*TM(I3,J) 
      RETURN
      END   
C-----------------------------------------------------------tb-
      SUBROUTINE SETRGB(X,Y,Z)  
C   
...
C    dummy routines for consistency with xpak.c
C
      SUBROUTINE UPDATE
      END
