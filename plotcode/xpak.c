...
CreateColormap( dsply,scrn,red,green,blue,irgbv,steps )
Display *dsply;
Screen *scrn;
...
int steps;
{
    ...
    unsigned long pixels[MAX_COLOURS],plane_mask[MAX_COLOURS];
     ...
     */
    result=1;
/* to generate cmap for wx
   reset PLOT_COLOURS and USEFUL_SIZE in cmndefs.h
    for (i=0;i<col_end;i++) 
        printf("%d,%d,%d,%d\n",Colours[i].red/256,Colours[i].green/256,Colours[i].blue/256,(i==0||i==col_end-1)?1:0);
*/
...
/*------------------------------------------------------*/
symbol_(x,y,size,iword,angl,nchar)
...
{
static float     xsiz;
int              len,ascent,descent;
float            xp, yp, ixv, iyv;
short            i;
static XFontStruct *currfont;
 ...
 */
fillpoly_(xf,yf,nf,colour,outline)
float *xf, *yf;
 ...
 *****************************************************************/
Translate_u_to_win(xu,yu,xwin,ywin)
float     xu,yu;
int     *xwin,*ywin;
{
    float     tmpx,tmpy;
...
int drawticmarks_(size,max,val,start,interval,justify,horizontal)
int *justify, *horizontal;
float *size, *max;
/*float *min,*max;*/
float *val,*start,*interval;
{
    int pen_state;
    float offset,xval,yval;
    /*float size;*/

    /*size = (*max - *min)/30.0;*/
    if (*justify==J_CENTRE) offset = -(*size)/2;
    else offset = 0;
    if (*justify==J_TOP || *justify==J_RIGHT) *size = -(*size);
    if (*horizontal) {
        for (xval = *start; xval < *max; xval += *interval) {
            pen_state = PEN_UP;
            yval = *val + offset;
        ...
        for (yval = *start; yval < *max; yval += *interval) {
            pen_state = PEN_UP;
            xval = *val + offset;
            plotu_(&xval,&yval,&pen_state);
            pen_state = PEN_DN;
...
#if XY
#endif
int drawsegment_(x1,y1,x2,y2,colour)
float *x1,*y1,*x2,*y2;
int *colour;
{
    int pen_state;
    ...
    XChangeGC(Dsply,Draw_gc,valuemask,&values);
    pen_state = PEN_UP;
    plotu_(x1,y1,&pen_state);
    pen_state = PEN_DN;
    plotu_(x2,y2,&pen_state);
    /* reset pen color */
    XChangeGC(Dsply,Draw_gc,valuemask,&oldvalues);
}
