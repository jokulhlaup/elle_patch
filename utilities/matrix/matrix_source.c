 ...
 *
 * Copyright (C) 1996, 1997, 1998, 1999, 2000 Gerard Jungman, Brian
 * Gough
 */
#include "view.h"

QUALIFIED_VIEW(_gsl_vector,view)
FUNCTION (gsl_matrix, column) (QUALIFIED_TYPE(gsl_matrix) * m, const
size_t j)
{
  QUALIFIED_VIEW(_gsl_vector,view) view = NULL_VECTOR_VIEW;
                                                                                
  if (j >= m->size2)
    {       GSL_ERROR_VAL ("column index is out of range", GSL_EINVAL,
view);
    }
