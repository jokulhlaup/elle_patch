...
#define P_EDGE(d, e, i, j) {(d)= (e); \
                            do {(d)= (d)->prev;} while (!(d)->outp); \
                            (i)= (d)->bot.x + (d)->delx * ((j)-(d)->bot.y);}

#define N_EDGE(d, e, i, j) {(d)= (e); \
...
typedef struct edge_shape
{
  ...
  double              x;            /* Line x coordinate for current y   */
                        ...
                        double xtop)
{
  ...
  else
  {
    ...
    if (GE(xtop, (*st)->xtop) || EQ(edge->delx, (*st)->e->delx))
    {
    ...
    else
    {
      ...
      den= (((*st)->e->top.x-(*st)->e->bot.x)*(edge->top.y-edge->bot.y)-
            ((*st)->e->top.y-(*st)->e->bot.y)*(edge->top.x-edge->bot.x));

      /* Check for valid intersection */
      if (EQ(den, 0.0))
      {
        /* Treat as parallel - insert edge here (before the ST edge) */
        existing_node= *st;
        MALLOC(*st, sizeof(st_node), "ST insertion");
        (*st)->e= edge;
        (*st)->xtop= xtop;
      ...
      {
        /* A real intersection */
        r= (((*st)->e->bot.y-edge->bot.y)*(edge->top.x-edge->bot.x)-
            ((*st)->e->bot.x-edge->bot.x)*(edge->top.y-edge->bot.y)) / den;
        x= (*st)->e->bot.x + r * ((*st)->e->top.x - (*st)->e->bot.x);
        y= (*st)->e->bot.y + r * ((*st)->e->top.y - (*st)->e->bot.y);

        /* Insert the intersection and the edge pointers in the IT */
        add_intersection(it, edge, (*st)->e, x, y);

        /* Head further into the ST */
        add_st_edge(&((*st)->prev), it, edge, xtop);
      }
    }
  }
}
...
static void build_intersection_table(it_node **it, edge_node *aet, double yt)
{
  st_node   *st, *stp;
  edge_node *edge;
  ...
  for (edge= aet; edge; edge= edge->next)
  {
    ...
    {
      if (EQ(edge->top.y, yt))
        add_st_edge(&st, it, edge, edge->top.x);
      else
        add_st_edge(&st, it, edge, edge->bot.x + edge->delx*(yt-edge->bot.y));
    }
  }
  while (st)
  {
                      ...
                      gpc_polygon *result)
{
  ...
  if ((subject->num_contours == 0) && (clip->num_contours == 0))
  {
  ...
  while (scanbeam < sbt_entries)
  {
    ...
    for (next_edge= aet->next; next_edge; next_edge= next_edge->next)
    {
      ...
      {
        if (EQ(e0->x, next_edge->x) && EQ(e0->delx, next_edge->delx)
	 && NE(e0->top.y, yb))
        {
      ...
      if (next_edge->bundle[BELOW][next_edge->type])
      {
        ...
        if (EQ(e1->x, next_edge->x) && EQ(tedx, tndx)
    ...
    if (scanbeam < sbt_entries)
    {
      ...
      build_intersection_table(&it, aet, yt);
      ...
      for (edge= aet; edge; edge= next_edge)
      {
        ...
        else
        {
          ...
          if (EQ(edge->top.y, yt))
            ...
            edge->x= edge->bot.x + edge->delx * (yt - edge->bot.y);
        }
      }
    }
                        ...
                        gpc_tristrip *result)
{
  ...
  int            class, bl, br, tl, tr;
  double        *sbt= NULL, xb, px, nx, yb, yt, dy, ix, iy, tedx, tndx;
        ...
        if (contributing)
        {
          ...
          case IMN:
            if (NE(cf->bot.y, yb) && NE(cf->top.y, yb))
              VERTEX(cf, LEFT, cf->x, yb);
            new_tristrip(&tlist, cf, cf->x, yb);
            edge->outp= cf->outp;
            VERTEX(edge, RIGHT, xb, yb);
            px= xb;
            cf= edge;
