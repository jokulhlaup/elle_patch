...
SET PATH=.;%CYGWIN_ROOT%\bin;%CYGWIN_ROOT%\home\sybil;%CYGWIN_ROOT%\usr\X11R6\bin;%PATH%
...
echo sybil.bat - Starting on Windows 95/98/Me
...
echo sybil.bat - Starting on Windows NT/2000/XP/2003
...
REM Check if Xwin already running
tasklist | %SYSTEMROOT%\System32\find "XWin.exe"
if %ERRORLEVEL% EQU 0 goto Xexists
...
:Xexists
 %RUN% xterm -e sybil %1 %2 %3 %4 %5 %6 %7 %8 %9
REM %RUN% xterm 
:END
