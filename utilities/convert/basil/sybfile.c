...
#include <sys/stat.h>
#include "sybfile.h"
#include "../../../plotcode/cmndefs.h"
#include "../../../plotcode/filedat.h"
#include "data.h"
#include "errnum.h"
#include "error.h"
#include "arrays.h"
...
/*
int file_open_w( fp,name )
FILE **fp;
char *name;
{
    int err=0;

    if ((*fp = fopen( name,"w" ))==NULL) err= OPEN_ERR;
    else {
    }
    return( err );
}
...
#ifdef SUN
int file_open( data, name, max, mode )
file_data **data;
char *name;
char *mode;
int max;
...
#ifdef SUN
int read_data( fp,string_vars,int_vars,fl_vars,
                arraysi,arraysf,xmin,xmax,
                ymin,ymax,current,req,maxrec,rotate )
FILE *fp;
char *string_vars;
int *int_vars, **arraysi; 
float *fl_vars,**arraysf,*xmin,*xmax,*ymin,*ymax; 
int *current,req,*maxrec,rotate;
#else
...
int read_data_blk( fp,string_vars,int_vars,fl_vars,arraysi,arraysf)
FILE *fp;
char *string_vars;
int *int_vars, **arraysi; 
float *fl_vars,**arraysf; 
{
    ...
    if (cnt!=REC_1_CNT*sizeof(char)) {
         ...
         */
            for (i=0;i<FUDGE_64BIT;i++) fread( &cnt, 1, sizeof(int), fp );
            if (cnt!=REC_1_CNT*sizeof(char)) {
                swap_4_bytes((char *)(&cnt));
                byte_swap = 1;
            }
...
int read_reference( fp,int_vars,arraysf,current,ref,req,rotate )
FILE *fp;
int *int_vars; 
float **arraysf; 
int current,*ref,req,rotate;
{
    int i,err=0,rec_num,rotate_velocity=0;
    long curr_pos;
...
int ReadIntArray(cnt,addr,fp,byte_swap)
int cnt;
int **addr;
FILE *fp;
...
int ReadFlArray(cnt,addr,fp,byte_swap)
int cnt;
float **addr;
FILE *fp;
unsigned char byte_swap;
